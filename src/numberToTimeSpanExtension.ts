...
namespace Humanizer {
  ...
  Number.prototype.time = function(
    percision: number = 1,
    countEmptyUnits: boolean = false,
    culture: string = Resources.getCurrentCulture(),
    maxUnit: Localisation.TimeUnit = Localisation.TimeUnit.Week
  ): string {
  ...
  function parts(
    timespan: number,
    culture: string,
    maxUnit: Localisation.TimeUnit
  ): string[] {
    var days: number = timespan / MILLIS_PER_DAY
    var weeks: number = Math.floor(days / 7)
    var daysInWeek: number =
      maxUnit > Localisation.TimeUnit.Day ? days % 7 : Math.floor(days)
    ...
    var milliseconds: number =
      maxUnit === Localisation.TimeUnit.Millisecond
        ? timespan
        : timespan - seconds * MILLIS_PER_SECOND

    var outputWeeks: boolean =
      weeks > 0 && maxUnit === Localisation.TimeUnit.Week
    var outputDays: boolean =
      (outputWeeks || daysInWeek > 0) && maxUnit >= Localisation.TimeUnit.Day
    var outputHours: boolean =
      (outputDays || hours > 0) && maxUnit >= Localisation.TimeUnit.Hour
    var outputMinutes: boolean =
      (outputHours || minutes > 0) && maxUnit >= Localisation.TimeUnit.Minute
    var outputSeconds: boolean =
      (outputMinutes || seconds > 0) && maxUnit >= Localisation.TimeUnit.Second
    var outputMilliseconds: boolean =
      (outputSeconds || milliseconds > 0) &&
      maxUnit >= Localisation.TimeUnit.Millisecond

    var result: string[]
    var formatter: Localisation.Formatter.IFormatter = Configuration.Configurator.getFormatter(
      culture
    )
    if (outputWeeks) {
    ...
    if (outputMilliseconds) {
      result.push(
        part(formatter, Localisation.TimeUnit.Millisecond, milliseconds)
      )
    } else {
  ...
  function part(
    formatter: Localisation.Formatter.IFormatter,
    timeUnit: Localisation.TimeUnit,
    unit: number
  ): string {
