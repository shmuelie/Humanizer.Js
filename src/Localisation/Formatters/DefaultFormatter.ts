namespace Humanizer.Localisation.Formatter {
  'use strict'
  ...
  function getResourceForDate(
    unit: TimeUnit,
    timeUnitTense: Tense,
    count: number,
    culture: string
  ): string {
    var resourceKey: string = ResourceKeys.DateHumanize.GetResourceKey(
      unit,
      timeUnitTense,
      count
    )
    return count === 1
      ? format(resourceKey, culture)
      : format(resourceKey, culture, count)
  }

  function getResourceForTime(
    unit: TimeUnit,
    count: number,
    culture: string
  ): string {
    var resourceKey: string = ResourceKeys.TimeHumanize.GetResourceKey(
      unit,
      count
    )
    return count === 1
