...
namespace Humanizer {
  ...
  function addIrregluar(singular: string, plural: string): void {
    addPlural(
      '(' + singular.charAt(0) + ')' + singular.substr(1) + '$',
      '\\1' + plural.substr(1)
    )
    addSingular(
      '(' + plural.charAt(0) + ')' + plural.substr(1) + '$',
      '\\1' + singular.substr(1)
    )
  }
  ...
  function applyRules(
    rules: Array<InflectorExtensionsRule>,
    word: string
  ): string {
    if (word === null) {
      return null
  ...
  String.prototype.underscore = function(): string {
    ...
    return this.replace(/([A-Z]+)([A-Z][a-z])/, '$1_$2')
