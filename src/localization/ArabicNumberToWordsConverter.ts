import { GenderedNumberToWordsConverter } from './GenderedNumberToWordsConverter'

const arabicGroup: string[] = ["مئة", "ألف", "مليون", "مليار", "تريليون", "كوادريليون", "كوينتليون", "سكستيليون"];
const arabicAppendedGroup: string[] = ["", "ألفاً", "مليوناً", "ملياراً", "تريليوناً", "كوادريليوناً", "كوينتليوناً", "سكستيليوناً"];
const arabicPluralGroups: string[] = ["", "آلاف", "ملايين", "مليارات", "تريليونات", "كوادريليونات", "كوينتليونات", "سكستيليونات"];
const onesGroup: string[] = ["", "واحد", "اثنان", "ثلاثة", "أربعة", "خمسة", "ستة", "سبعة", "ثمانية", "تسعة", "عشرة", "أحد عشر", "اثنا عشر", "ثلاثة عشر", "أربعة عشر", "خمسة عشر", "ستة عشر", "سبعة عشر", "ثمانية عشر", "تسعة عشر"];
const tensGroup: string[] = ["", "عشرة", "عشرون", "ثلاثون", "أربعون", "خمسون", "ستون", "سبعون", "ثمانون", "تسعون"];
const hundredsGroup: string[] = ["", "مئة", "مئتان", "ثلاث مئة", "أربع مئة", "خمس مئة", "ست مئة", "سبع مئة", "ثمان مئة", "تسع مئة"];
...
export class ArabicNumberToWordsConverter extends GenderedNumberToWordsConverter {
    convert_number(num: number): string {
        ...
        while (num >= 1) {
            const groupNumber: number = num % 1000;
            num = Math.floor(num / 1000);

            let tens: number = groupNumber % 100;
            const hundreds: number = Math.floor(groupNumber / 100);
            let process: string = "";
