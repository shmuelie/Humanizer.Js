...
const unitsMap: string[] = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen", "seventeen", "eighteen", "nineteen"];
const tensMap: string[] = ["zero", "ten", "twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety"];
const exceptions: ExceptionsDictionary = {
    1: "first",
    2: "second",
    3: "third",
    4: "fourth",
...
function getUnitValue(num: number, isOrdinal: boolean): string {
    if (isOrdinal) {
        const exception: string = exceptions[num];
        if (exception) {
            return exception;
        }
        else {
...
function convert(num: number, isOrdinal: boolean): string {
    ...
    if (num > 0) {
        ...
        if (num < 20) {
            ...
            let lastPart: string = tensMap[Math.floor(num / 10)];
            if ((num % 10) > 0) {
