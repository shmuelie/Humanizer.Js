...
export class DefaultCollectionFormatter implements ICollectionFormatter {
    ...
    protected humanize_collection<T>(collection: T[]): string {
        return this.humanizer_collection_objectFormatter_separator<T>(collection, (item: T): string => {
            return item.toString();
        }, this.defaultSeparator);
    }

    protected humanize_collection_separator<T>(collection: T[], separator: string): string {
        return this.humanizer_collection_objectFormatter_separator<T>(collection, (item: T): string => {
            return item.toString();
        }, separator);
    }

    protected humanizer_collection_objectFormatter<T>(collection: T[], objectFormatter: (item: T) => string): string {
        return this.humanizer_collection_objectFormatter_separator<T>(collection, objectFormatter, this.defaultSeparator);
    }

    protected humanizer_collection_objectFormatter_separator<T>(_collection: T[], _objectFormatter: (item: T) => string, _separator: string): string {
